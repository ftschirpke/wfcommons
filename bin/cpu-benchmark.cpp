 ...
 * is that it does not cause extra memory references, and thus keeps this
 * benchmark 100% CPU intensive.
 */
void compute_terrible_pi(long work) {
  ...
  for (long sample = 0; sample < work; sample++) {
    ...
    terrible_pi +=
        (double)(std::sqrt(x_value * x_value + y_value * y_value) < 0.5);
  }
...
int main(int argc, char **argv) {
  ...
  std::cout << "CPU-Benchmark Main Start" << std::endl;
  ...
  std::cout << "CPU-Benchmark Main Checking Arguments" << std::endl;
  ...
  std::cout << "Start computing pi with " << work << "M samples...\n";
