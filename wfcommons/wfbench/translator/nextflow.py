...
"""
    ...
    def translate(self, output_folder: pathlib.Path) -> None:
        ...
        self.script += self._generate_task_id_code()

        self.abstract_tasks, self.abstract_parents = self._analyze_abstract_relationships()

        self.script += self._determine_input_output()

        self.workflow_inputs = set()
        for task in self.tasks.values():
            for infile in task.input_files:
                self.workflow_inputs.add(infile)
        for task in self.tasks.values():
            for outfile in task.output_files:
                self.workflow_inputs.remove(outfile)
    ...
    def _analyze_abstract_relationships(self) -> None:
        """
        Determines the abstract tasks that will be used for the nextflow definition of the workflow.
        """

        abstract_tasks = defaultdict(list)
        abstract_parents: Dict[str, MutableSet[str]] = defaultdict(set)
        ...
        for abstract_task in abstract_tasks:
            for abstract_parent in abstract_parents[abstract_task]:
        ...
        return abstract_tasks, abstract_parents

    def _determine_input_output(self) -> None:
        """
        Determines the inputs and outputs for the physical and abstract tasks.
        """
        task_input_amounts: Dict[str, int] = {}
        script_addition: str = ""

        script_addition += "// define amount of input files for abstracts tasks where the amount is not constant\n"
        for abstract_task_name, physical_tasks in self.abstract_tasks.items():
            input_amounts = {task.task_id: len(task.input_files) for task in physical_tasks}
            if (max(input_amounts.values()) == min(input_amounts.values())):
                # all physical tasks have the same amount of inputs
                task_input_amounts[abstract_task_name] = max(input_amounts.values())
            else:
                # define amount of inputs for each physical task
                script_addition += f"def {self.valid_task_name(abstract_task_name)}_input_amounts = [\n"
                for k, v in input_amounts.items():
                    script_addition += f"  \"{k}\": {v},\n"
                script_addition += "]\n"
                task_input_amounts[abstract_task_name] = None
        script_addition += "\n"
    ...
    def _create_workflow_script(self):
        """
...
'''
    ...
    def _generate_task_id_code(self) -> str:
        return """
